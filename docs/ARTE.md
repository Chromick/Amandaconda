...
> **Nota (Godot 3D):** o runtime oficial é mesh/3D. Este doc (e a pasta `docs/arte/`)
> serve de **referência de personagem, paleta e mood** — não como pipeline de sprites 2D.

Você desenha, o jogo carrega. Este documento existe para que essas duas coisas se encontrem
sem retrabalho.

A tabela de tamanhos e a lista de animações do protótipo 2D viviam em `data/sprites.json`
(arquivo morto). O que está escrito aqui explica **por quê** as regras de arte importam.
