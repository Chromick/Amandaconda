...
func setup(color: Color = Color(1.0, 0.85, 0.35), scale_u: float = 1.0) -> void:
	...
	light.light_energy = 2.5 * scale_u
	light.omni_range = 2.5 * scale_u
	add_child(light)
	var tw := create_tween()
	...
	for i in 6:
		...
		tw.tween_property(shard, "position", dir * randf_range(0.45, 0.9) * scale_u, 0.18)
		tw.tween_property(shard, "scale", Vector3.ZERO, 0.18)
