...
func take_damage(amount: float, knockback: Vector3 = Vector3.ZERO, source: Node = null) -> void:
	...
	if not _phase2 and health <= max_health * float(_cfg.get("fase2_vida", 0.5)):
		...
		_spawn_puddle()
		HitFeel.shake(0.35)
		HitFeel.spark_at(global_position + Vector3.UP * 1.2, Color(1.0, 0.55, 0.15), 1.4)
		HitFeel.spark_at(global_position + Vector3.UP * 0.3, Color(1.0, 0.7, 0.25), 0.9)
		HitFeel.kick_fov(6.0, 0.2)
		GameState.show_toast("LUANEVIL · caramelo derrete")
		scale = Vector3(1.18, 0.88, 1.18)
		var tw := create_tween()
		tw.tween_property(self, "scale", Vector3.ONE, 0.2).set_trans(Tween.TRANS_BACK).set_ease(Tween.EASE_OUT)
